export default function sandsplines (p) {
  p.setup = function() {
    ...
    for (var i = 0; i < p.height * 1; i += inc) {
      ...
      for (var j = 0; j < 8; j++) {
        ...
        p.stroke(205, 185, 105, t); // change to h, 200, 200, t
  ...
  p.u = function(uu) {
    return (p.width / 50) * uu;
  }

  p.drawLine = function(x1, x2, volatility) {
    var granularity = p.random(100, 1000); // this controls the number of points per line
    // noise offsets
    var yoff = 0;
    var xoff = 1000;
    var w = x2 - x1;
    ...
    for (var i = 1; i < granularity; i++) {
      var y = p.map(p.noise(yoff), 0, 1, -p.u(50), p.u(50)) * (volatility * p.noise(i / 150));
      var x = (w / granularity) * i;

      x += p.map(p.noise(xoff), 0, 1, -p.u(60), p.u(60));
      y *= p.exp(i * 0.01) * 0.015;
      p.curveVertex(x, y);
      yoff += 0.009;
      xoff += 0.001;
    }
